  ...
  return scheduleStorage.createSchedule(scheduleObject)
    .then(scheduleDocument => {
      ...
      return Promise.all(promiseArray)
        .then(() => Promise.resolve(scheduleDocument))
    })
}
...
const _updateSchedule = (scheduleStorage, oldScheduleDocument, newScheduleDocument) => {
  ...
  // Check if the new document is changing the trvsAppliedOn property
  if (newScheduleDocument.trvsAppliedOn) {
    if (oldScheduleDocument.trvsAppliedOn.length < newScheduleDocument.trvsAppliedOn.length) {
      // need to add the schedule id to a trv
    }

    if (oldScheduleDocument.trvsAppliedOn.length > newScheduleDocument.trvsAppliedOn.length) {
      // need to remove the schedule id from a trv
    }
  }
