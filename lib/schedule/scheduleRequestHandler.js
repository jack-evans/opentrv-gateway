...
const logger = bunyan.createLogger({name: 'schedule-request-handler', serializers: bunyan.stdSerializers})
...
const _createSchedule = (scheduleStorage, schedule) => {
  ...
  let scheduleObject = new Schedule(schedule.name, schedule.targetTemperature, schedule.startTime, schedule.endTime, schedule.devicesAppliedOn)
