 ...
 * @param  {Object} scheduleDocument the JSON document of the schedule to be stored
 * @return {Promise}                 on the action of storing the schedule document
 * @private
 */
const _createSchedule = (scheduleStorage, scheduleDocument) => {
  logging.logFunctionEntry(logger, '_createSchedule', true, scheduleDocument)
  ...
  if (typeof scheduleDocument !== 'object') {
    error.statusCode = 400
    error.message = 'The body provided was not an object... It was of the type ' + typeof scheduleDocument
    error.name = 'bad request'
    return Promise.reject(error)
  }

  return scheduleStorage.createSchedule(scheduleDocument)
}
 ...
 * @param  {Object} scheduleStorage     the storage medium for the schedules
 * @param  {Object} newScheduleDocument the new storage document to update with
 * @return {Promise}                    on the action of updating the document
 */
const _updateSchedule = (scheduleStorage, newScheduleDocument) => {
