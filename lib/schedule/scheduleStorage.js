...
scheduleStorage.prototype.createSchedule = (schedule) => {
  ...
  return new Promise((resolve, reject) => {
    jsonFile.writeFile(path.join(scheduleStoragePath, fileName), schedule, {spaces: 2}, function (err) {
      if (err) {
        reject(err)
      } else {
        resolve()
...
scheduleStorage.prototype.updateSchedule = (schedule) => {
  return new Promise((resolve, reject) => {
    jsonFile.readFile(path.join(scheduleStoragePath, (schedule.id + '.json')), function (err, data) {
      if (err) {
        reject(err)
      } else {
        let currentSchedule = data
        let scheduleToSave = Object.assign(currentSchedule, schedule)
        jsonFile.writeFile(path.join(scheduleStoragePath, (schedule.id + '.json')), scheduleToSave, {spaces: 2}, function (err) {
          if (err) {
            reject(err)
          } else {
            resolve()
...
scheduleStorage.prototype.deleteSchedule = (scheduleId) => {
  const fileName = scheduleId + '.json'
  return new Promise((resolve, reject) => {
    fs.unlink(path.join(scheduleStorage, fileName), function (err) {
      if (err) {
        reject(err)
      } else {
        resolve()
