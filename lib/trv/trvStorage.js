      ...
      if (err) {
        ...
        if (numOfItems > 0) {
          items.forEach((item, index) => {
            jsonFile.readFile(path.join(trvStoragePath, item), function (err, data) {
              if (err) {
                reject(err)
              } else {
                result.push(data)
...
trvStorage.prototype.updateTrv = (trv) => {
  return new Promise((resolve, reject) => {
    jsonFile.readFile(path.join(trvStoragePath, trv.id), function (err, data) {
      if (err) {
        reject(err)
      } else {
        let currentTrv = data
        let trvToSave = Object.assign(currentTrv, trv)
        jsonFile.writeFile(path.join(trvStoragePath, trv.id), trvToSave, {spaces: 2}, function (err, result) {
          if (err) {
            reject(err)
          } else {
            resolve(result)
