...
const _updateTrv = (trvStorage, oldTrv, newTrv) => {
  ...
  if (typeof newTrv !== 'object') {
    error.statusCode = 400
    error.message = 'The body provided was not an object. It was of the type ' + typeof newTrv
    error.name = 'bad request'
    return Promise.reject(error)
  }
...
const patchActiveSchedulesRequestHandler = (req, res) => {
  logging.logFunctionEntry(logger, 'patchActiveSchedulesRequestHandler', false, {body: req.body})
  module.exports.internal._getTrvByID(req.trvStorage, req.params.id)
    .then(trv => {
      return module.exports.internal._patchActiveSchedules(req.trvStorage, trv, req.body)
    })
    .then(trv => {
      logger.info('Successfully updated the schedules property in the trv ', trv)
      res.status(200).send(trv)
    })
    .catch(error => {
      logging.logErrorCase(logger, 'patchActiveSchedulesRequestHandler', error)
      switch (error.statusCode) {
        case 400: {
          res.status(400).send(error)
          break
...
const _patchActiveSchedules = (trvStorage, oldTrv, reqBody) => {
  logging.logErrorCase(logger, '_patchActiveSchedules', true, {oldTrv: oldTrv, body: reqBody})
  let error = {}
  if (!reqBody) {
    error.statusCode = 400
    error.message = 'The body provided was undefined'
  ...
  if (!reqBody.scheduleId) {
    error.statusCode = 400
    error.message = 'The scheduleId provided was undefined'
    error.name = 'bad request'
    return Promise.reject(error)
  }

  if (!reqBody.operation) {
    error.statusCode = 400
    error.message = 'The operation was not provided'
    error.name = 'bad request'
    return Promise.reject(error)
  }

  if (reqBody.operation !== 'delete' && reqBody.operation !== 'add') {
    error.statusCode = 400
    error.message = 'The operation provided was not delete or add'
    error.name = 'bad request'
    return Promise.reject(error)
  }

  let newTrv = JSON.parse(JSON.stringify(oldTrv))
  let index = newTrv.activeSchedules.indexOf(reqBody.scheduleId)

  if (reqBody.operation === 'delete') {
    if (index < 0) {
      error.statusCode = 404
      error.message = 'The scheduleId does not exist in the array'
      error.name = 'not found'
      return Promise.reject(error)
    } else {
      newTrv.activeSchedules.splice(index, 1)
    }
  }

  if (reqBody.operation === 'add') {
    if (index > -1) {
      error.statusCode = 409
      error.message = 'The scheduleId provided is already in the array'
      error.name = 'conflict'
      return Promise.reject(error)
    } else {
      newTrv.activeSchedules.push(reqBody.scheduleId)
    }
  }

  return module.exports.internal._updateTrv(trvStorage, oldTrv, newTrv)
}
...
module.exports = {
  ...
  patchActiveSchedulesRequestHandler: patchActiveSchedulesRequestHandler
}
...
module.exports.internal = {
  ...
  _patchActiveSchedules: _patchActiveSchedules
}
