...
const logErrorCase = (functionName, errorObject) => {
  ...
  logger.info(logMessage, errorObject || '')
}
...
const _getTrvActivity = (trvStorage, id) => {
  logFunctionEntry('_getTrvActivity', true, {id: id})
  let error = {}
  if (!id) {
    error.statusCode = 400
    error.message = 'The id provided was undefined'
...
const getTrvTemperatureRequestHandler = (req, res) => {
  logFunctionEntry('getTrvTemperatureRequestHandler', false, {id: req.id})
  let trvStorage = req.trvStorage
...
const _getTrvTemperature = (trvStorage, id) => {
  logFunctionEntry('_getTrvTemperature', true, {id: id})
  let error = {}
  if (!id) {
    error.statusCode = 400
    error.message = 'The id provided was undefined'
  ...
  return trvStorage.getTrvById(id)
    .then(trv => {
      Promise.resolve(trv.currentTemperature)
    })
    .catch(error => {
      Promise.reject(error)
    })
